...
SELECT COUNT(*) 'Кол-во сотрудников' FROM staff
...
SELECT date_activity, COUNT(staff_id) AS 'Кол-во вышедших сотрудников' FROM activity_staff
GROUP BY date_activity
HAVING COUNT(staff_id) > 3;

-- Выведите среднюю заработную плату по должностям, которая составляет более 30000
SELECT post, AVG(salary) 'Средняя з/плата' FROM staff
GROUP BY post
